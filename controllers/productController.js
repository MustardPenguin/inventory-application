...
exports.product_update = (req, res, next) => {
    async.parallel(
        ...
        (err, results) => {
            ...
            for(const catalog of results.catalogs) {
                if(results.product.catalog.includes(catalog._id)) {
                    catalog.checked = true;
                }
            }
            console.log(results);
            res.render("create_product", {
                ...
                catalogs: results.catalogs,
                product: {
                    product_name: results.product.name,
                    product_quantity: results.product.quantity,
                    product_description: results.product.description,
                    product_price: results.product.price,
                    product_seller: results.product.seller,
                },
            });
        }
    );
...
exports.product_update_post = [
    ...
    (req, res, next) => {
        ...
        Product.findByIdAndUpdate(req.params.id, product, {}, (err) => {
            ...
            res.redirect('/inventory/product/' + req.params.id);
        });
    }
];